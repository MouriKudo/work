"""Generate visualization figures from metadata.csv"""
import sys
import pandas as pd
import numpy as np
...
def plot_hu_distribution(metadata_df, output_path):
    ...
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"HU distribution saved to {output_path}")
...
def plot_class_imbalance(metadata_df, output_path):
    ...
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"Class imbalance saved to {output_path}")

# Run
plot_sample_grid(PATCHES_DIR, df, str(FIGURES_DIR / "patch_samples_grid.png"))
plot_hu_distribution(df, str(FIGURES_DIR / "hu_distribution.png"))
plot_class_imbalance(df, str(FIGURES_DIR / "class_imbalance.png"))
